...
class C_Player extends C_Entity{
	...
	C_Player(){
		...
		OffHand = null;
		BulletTimer=0;
		ReloadTimer=MainHand.ReloadTime;
		dwShot=0;
		dir=1;
		Throw=false;
		lives=3;
		...
		isShooting=0;
		killerID="00";
		Nickname=C_SaveOptions.Open();//"Master Exploder";
		if (Nickname == null)
			Nickname = "Unnamed";
	...
	void Step(Iterator _it) {
		...
		if (isSpawned){
			...
			if (hp<=0) {
				...
				Reload=false;
				ReloadTimer=SpawnWeapon.ReloadTime;
				BulletTimer=0;
				Grenades=2;
			}
			if (!yourDead){
				if (!isSniper){
					...
					camxv=(float) (Math.cos(camdir))*camdis/15;
					camyv=(float) (Math.sin(camdir))*camdis/15;
					camx+=camxv;
					camy+=camyv;
			...
			if (mouse_button==3 && !mousedown && Grenades>0){
				C_Grenade asdf=new C_Grenade((int)x, (int)y, 60);
				asdf.Shoot(null, C_Global.Mouse.x+C_Global.Camera_x, C_Global.Mouse.y+C_Global.Camera_y, 0);
				C_Global.World.Create(asdf);
				CO_Client.send("22", (int)x+"!"+(int)y+"@"+C_Global.Mouse.x+C_Global.Camera_x+"#"+C_Global.Mouse.y+C_Global.Camera_y+"$");
				Grenades--;
				mousedown=true;
			}
